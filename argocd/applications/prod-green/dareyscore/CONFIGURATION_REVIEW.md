...
**Status**: Resolved. Deployments now consume all database credentials directly from `dareyscore-api-secrets`.

### 3. Redis Password Configuration
**Status**: Resolved. `dareyscore-api-secrets` exposes `REDIS_URL` and associated TLS metadata, and deployments consume the secret via `envFrom`.
