...
1. Namespace
2. ExternalSecrets (populate managed RDS/Redis credentials from AWS Secrets Manager)
3. ConfigMap
4. Migration Job
5. API Deployment
6. Worker Deployment
...
- **ExternalSecret**: `dareyscore-api-secrets` (sensitive data from AWS Secrets Manager)

### Secrets Management

Secrets are stored in AWS Secrets Manager and synced to Kubernetes via External Secrets Operator:

- `prod/dareyscore/postgres`: Managed PostgreSQL connection details (host, port, db, user, password, sslmode)
...
- `prod/dareyscore/redis`: Managed Redis connection settings (URL, password, TLS flag)
...
# Fetch current secret values (sanity check)
kubectl get secret -n dareyscore dareyscore-api-secrets -o yaml

# Validate managed RDS connectivity from a pod
kubectl exec -n dareyscore -it <api-pod-name> -- psql "$DATABASE_URL"
```
